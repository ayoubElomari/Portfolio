 ...
 *
 * ── Gated, like the bench ────────────────────────────────────────────────────
 *
 * The kit this was copied from (`.project-details/rendr/v2/live-demos/`)
 * auto-starts this size by default — one video source is cheap enough not to
 * ask permission for. This project's copy deliberately deviates from that
 * default and gates it behind the same "Run the engine" button as the bench,
 * so the reader chooses when the decoder spins up rather than it happening the
 * moment the demo scrolls into view. That is a per-project choice, not a bug
 * fix — leave the shared kit's own default alone if you copy it elsewhere.
 */
export default function MediumDemo({
  ...
  /* The runtime does not start until it is asked for — see the note at the top
     of this file. Fetches the video, demuxes it and stands up a decoder; that
     is a real cost to hand somebody who came to read the page. */
      ...
      <div className="rendr-medium">
        ...
        >
          ...
          {/* Before the engine exists: what it is, what it will cost, and the
              one control that starts it — same gate as the bench. */}
          {!started && (
            <div className="md-stage-gate">
              <button
                type="button"
              ...
              <p className="md-gate-note">{t.gateNote}</p>
            </div>
          )}
