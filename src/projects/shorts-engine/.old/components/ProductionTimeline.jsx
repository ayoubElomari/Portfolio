 ...
 * fact that CutFun is a single mark and never a lane. The scatter between
...
/* Cell centre as a percentage of the track, for anything that has to line up
   with a specific day (month ticks, the CutFun tag). */
...
const LANES = [
  {
    ...
    label:
      "QuickLore: 255 videos across 82 production days, July 21 to October 27, 2025.",
  ...
  },
  {
    ...
    label:
      "GesoLore: 161 videos across 60 production days, August 17 to October 29, 2025.",
    cells: lane(day(8, 17), day(10, 29), 60, 4877),
  },
  {
    name: "CutFun",
    meta: "1 video · 1 day",
    label: "CutFun: a single video on July 25, 2025, and nothing after it.",
...
export default function ProductionTimeline() {
  return (
    <div className="se-tl">
      ...
      {LANES.map((row) => (
        <div className="se-tl-lane" key={row.name}>
          <span className="se-tl-head">
            <span className="se-tl-name">{row.name}</span>
            <span className="se-tl-meta">{row.meta}</span>
          </span>

          <span className="se-tl-track" role="img" aria-label={row.label}>
      ...
      <p className="se-tl-foot">
        One cell = one day, July 21 to October 29, 2025. Filled cells are days a
        channel actually shipped.
      </p>
    </div>
  );
}
